...
function cargarBienvenidaRh() {
    ...
    // Tomamos el primer icono de las opciones (o uno principal coherente) para la cabecera de bienvenida
    const iconoPrincipal = window.cirnorhConfig.options[0].icon;
...
// Funciones de visualización específicas sincronizadas con los SVGs correctos
function cargarPersonalRh() {
    const svgIcon = window.cirnorhConfig.options.find(o => o.id === 'personal').icon;
    document.getElementById('app-container').innerHTML = `
...
function cargarAsistenciaRh() {
    const svgIcon = window.cirnorhConfig.options.find(o => o.id === 'asistencia').icon;
    document.getElementById('app-container').innerHTML = `
        <div class="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm animate-fade-in">
            <div class="flex items-center gap-3 mb-4">
...
function cargarVacacionesRh() {
    const svgIcon = window.cirnorhConfig.options.find(o => o.id === 'vacaciones').icon;
    document.getElementById('app-container').innerHTML = `
        <div class="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm animate-fade-in">
            <div class="flex items-center gap-3 mb-4">
...
function cargarCapacitacionRh() {
    const svgIcon = window.cirnorhConfig.options.find(o => o.id === 'capacitacion').icon;
    document.getElementById('app-container').innerHTML = `
        <div class="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm animate-fade-in">
            <div class="flex items-center gap-3 mb-4">
...
function cargarExpedientesRh() {
    const svgIcon = window.cirnorhConfig.options.find(o => o.id === 'expedientes').icon;
    document.getElementById('app-container').innerHTML = `
        <div class="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm animate-fade-in">
            <div class="flex items-center gap-3 mb-4">
...
function cargarGenerarOficiosRh() {
    const svgIcon = window.cirnorhConfig.options.find(o => o.id === 'generar-oficios').icon;
    document.getElementById('app-container').innerHTML = `
        <div class="bg-white rounded-2xl p-6 border border-stone-200 shadow-sm animate-fade-in">
            <div class="flex items-center gap-3 mb-4">
                <div class="p-2.5 bg-[#f0fdf4] text-[#059669] rounded-xl border border-[#c6f6d5]">
