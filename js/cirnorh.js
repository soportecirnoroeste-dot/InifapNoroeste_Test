...
function cargarBienvenidaRh() {
    ...
    if (window.cirnorhConfig && window.cirnorhConfig.options) {
        window.cirnorhConfig.options.forEach((opt) => {
            const bgSolo = opt.colorClass.split(' ')[0]; // Extrae solo el color de fondo para el contenedor del icono
            tarjetasHtml += `
...
// Funciones individuales de cada módulo usando directamente su propia configuración
...
// Función auxiliar para renderizar los módulos estándar y evitar repetir código
