...
exportarExcelCasc: function () {
        ...
        chavesGrupos.forEach(key => {
            ...
            const nombrePestana = escaparXml(`Emp_${etiquetaEmp}`.replace(/[\*\?\/\\\\[\]]/g, '')).substring(0, 31);
            const etiquetaEmpSegura = escaparXml(etiquetaEmp);
        ...
        <Worksheet ss:Name="${nombrePestana}">
            <Table>
                ...
                <Row>
                    <Cell ss:Index="3" ss:MergeAcross="${MaxCol - 3}" ss:StyleID="TitleMeta"><Data ss:Type="String">INCIDENCIAS DEL EMPLEADO: ${etiquetaEmpSegura}</Data></Cell>
                </Row>
                <Row>
            ...
            rawHeaders.forEach(h => {
                xmlContent += `        <Cell ss:StyleID="HeaderTable"><Data ss:Type="String">${escaparXml(h)}</Data></Cell>\n`;
            });
            ...
            rowsMapeadas.forEach(dRow => {
                ...
                dRow.forEach(cellVal => {
                    const safeVal = escaparXml(cellVal);
        ...
        try {
            ...
            alert("Ocurrió un error al generar la descarga del archivo.");
        }
    }
};